...
1. **Planning and execution are phase-split; subagent-driven development remains the default execution mode.** After saving a plan, `writing-plans` hands execution to a brand-new session — never the authoring one — with a pointer-only prompt (plan file path, worktree/branch, and the required sub-skill); the fresh session reads the plan file itself. It still never asks which execution approach to use. `executing-plans` is the fallback for harnesses without subagent access.
